...
export default function OwnerEditCommandScreen() {
  ...
  return (
    <OperationalScreenShell
      ...
      subtitle={
        commandId
          ? `Atualize os dados operacionais da comanda #${commandId}.`
          : "Atualize os dados operacionais da comanda."
      }
      ...
      topSlot={
        !isLoading && !error && commandQuery.data ? (
          <View className="flex-row gap-3">
            <OperationalMetricCard
              label="Status atual"
              value={
                commandQuery.data.status === "closed" ? "Fechada" : "Aberta"
              }
              helper={
                commandQuery.data.paymentMethod
                  ? getPaymentMethodLabel(commandQuery.data.paymentMethod)
                  : "Sem pagamento"
              }
              featured={commandQuery.data.status === "closed"}
            />
            <OperationalMetricCard
              label="Total atual"
              value={formatCurrency(commandQuery.data.total)}
              helper={`${commandQuery.data.items.length} item(ns)`}
            />
          </View>
        ) : undefined
      }
